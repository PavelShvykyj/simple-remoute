...
export const goodsReducer = createReducer(
  ...
  on(GoodsActions.addGoods, (state, { good }) =>
    goodsAdapter.addOne(good, state)
  ),
  on(GoodsActions.updateGoods, (state, { goods }) =>
    goodsAdapter.updateOne({ id: goods.id, changes: goods }, state)
  ),
  on(GoodsActions.deleteGoods, (state, { id }) =>
    goodsAdapter.removeOne(id, state)
  ),
  on(GoodsActions.selectFolder, (state, { id }) => ({
    ...state,
    currentFolder: id,
  })),
  on(GoodsActions.selectSearchValue, (state, { value }) => {
    const reg = '.*' + value.trim().toUpperCase().replace(/\s+/g, '.*') + '.*';
    return { ...state, searchValue: value.length >= 3 ? reg : '' };
  })
);
